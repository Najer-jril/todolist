                                ...
                                <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                    @foreach ($tasks as $task)
                                        <tr>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">{{ $task->category->category_name ?? 'Tanpa Kategori' }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap">{{ $task->due_date ? $task->due_date->format('d-m-Y') : '-' }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap capitalize">{{ $task->priority ?? '-' }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full
                                                    @if($task->status == 'pending') bg-gray-200 text-gray-800 @endif
                                                    @if($task->status == 'in progress') bg-yellow-200 text-yellow-800 @endif
                                                    @if($task->status == 'completed') bg-green-200 text-green-800 @endif">
                                                    {{ $task->status }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                                <a href="{{ route('tasks.show', $task->id) }}" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-900">Lihat</a>
                                                <a href="{{ route('tasks.edit', $task->id) }}" class="ml-4 text-yellow-600 dark:text-yellow-400 hover:text-yellow-900">Edit</a>
                                                <form class="inline-block ml-4" action="{{ route('tasks.destroy', $task->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus tugas ini?')">
                                                    @csrf
                                                    @method('DELETE')
